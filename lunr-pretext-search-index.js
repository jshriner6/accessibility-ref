...
var ptx_lunr_docs = [
{
  ...
  "body": " Open Source Textbooks    Use Case  Provide a free resource to students that has an option to be modified by the instructor.    Time Commitment  Minimal to significant. This will mostly depend on the level of modification desired.     Finding Existing Textbooks  There are numerous existing open source textbooks available in mathematics. The American Institute of Mathematics maintains a list of approved textbooks . Dana Ernst's list also has textbooks covering a variety of topics. An internet search for a specific topic may also be fruitful.  Be aware that \"open source\" does not imply \"accessible\", so it is necessary to evaluate these texts for accessibility. Textbooks that have an HTML option will, in general, be more accessible than textbooks that are available only as a PDF.  Modifying open source materials will vary based on how they were created. Look for instructions from the original author regarding creating derivatives of the original work.   "
},
{
  "id": "markup",
  "level": "1",
